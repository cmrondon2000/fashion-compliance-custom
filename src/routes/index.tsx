...
export const Route = createFileRoute("/")({
  head: () => ({
    meta: [
      { title: "Dashboard — Fashion API Compliance" },
      { name: "description", content: "Fashion compliance overview: products, suppliers, and certification status." },
    ],
  }),
  ...
  return (
    ...
    >
      ...
      <div className="relative mt-4 flex items-end justify-between gap-3">
        <div>
          <div className="text-[2rem] leading-none font-semibold tracking-tight tabular-nums text-foreground">
            {value}
          </div>
          {trend && (
            <div className={cn(
...
const statusColor: Record<ComplianceStatus, string> = {
  ...
  "non-compliant": "var(--color-destructive)",
};
...
function GlassCard({ children, className }: { children: React.ReactNode; className?: string }) {
  return (
    <div
      className={cn(
        "relative overflow-hidden rounded-2xl border border-white/40 dark:border-white/5 bg-white/60 dark:bg-white/[0.03] backdrop-blur-xl p-6 shadow-[0_1px_2px_rgba(15,23,42,0.04),0_8px_24px_-8px_rgba(15,23,42,0.08)]",
        className,
      )}
    >
      <div className="pointer-events-none absolute inset-x-0 top-0 h-px bg-gradient-to-r from-transparent via-white/80 to-transparent dark:via-white/20" />
      ...
      <div className="space-y-8 max-w-7xl mx-auto animate-fade-in">
        ...
        <div className="flex flex-wrap items-end justify-between gap-4">
          <div className="space-y-2">
            ...
            <h1 className="text-4xl font-semibold tracking-tight bg-gradient-to-br from-foreground to-foreground/60 bg-clip-text text-transparent">
              Welcome back
            </h1>
          ...
          <div className="flex gap-2">
            <Link
              to="/products"
              className="inline-flex items-center gap-1.5 rounded-xl border border-border bg-white/60 dark:bg-white/5 backdrop-blur px-3.5 py-2 text-sm font-medium hover:bg-accent transition-all hover:-translate-y-0.5"
            >
              Products <ArrowUpRight className="h-3.5 w-3.5" />
            </Link>
            <Link
              to="/suppliers"
              className="inline-flex items-center gap-1.5 rounded-xl bg-gradient-to-br from-indigo-500 via-violet-500 to-fuchsia-500 text-white px-3.5 py-2 text-sm font-medium shadow-lg shadow-violet-500/25 hover:shadow-violet-500/40 transition-all hover:-translate-y-0.5"
            >
        ...
        <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
          <StatCard
            ...
            sparkline={spark(products.length || 3)}
            delay={0}
          />
          <StatCard
            label="Suppliers"
            ...
            sparkline={spark(suppliers.length || 2)}
            delay={60}
          />
          <StatCard
            label="Compliance"
            ...
            trend={{ value: 2, up: rate >= 70 }}
            sparkline={spark(Math.max(1, rate / 12))}
            delay={120}
          />
          <StatCard
            label="Open issues"
        ...
        <div className="grid gap-6 lg:grid-cols-2">
          <GlassCard>
            <div className="flex items-center justify-between mb-4">
              ...
              <Link to="/products" className="text-xs font-medium text-muted-foreground hover:text-foreground inline-flex items-center gap-1 transition-colors">
          ...
          <GlassCard>
            <div className="flex items-center justify-between mb-4">
              ...
              <Link to="/suppliers" className="text-xs font-medium text-muted-foreground hover:text-foreground inline-flex items-center gap-1 transition-colors">
                All <ArrowUpRight className="h-3 w-3" />
              </Link>
            </div>
            <ul className="space-y-1">
