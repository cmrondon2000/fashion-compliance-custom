...
import {
  useProducts, useSuppliers, useAddProduct, useUpdateProduct, useDeleteProduct,
  deriveStatusFromSupplier, type ComplianceStatus, type Supplier, type Product,
} from "@/lib/store";
...
const productSchema = z.object({
  ...
  sku: z.string().trim().min(2, "SKU must be at least 2 characters").max(40).regex(/^[A-Za-z0-9_-]+$/, "Only letters, numbers, - and _"),
  ...
  return (
    <div className="flex items-center justify-between gap-3 border-t border-border px-5 py-3 bg-muted/20 text-xs text-muted-foreground">
      ...
      <div className="flex items-center gap-1">
        <button onClick={() => onPage(Math.max(1, page - 1))} disabled={page <= 1} className="inline-flex items-center gap-1 rounded-lg border border-border bg-background px-2.5 py-1.5 hover:bg-accent disabled:opacity-50 disabled:cursor-not-allowed">
          <ChevronLeft className="h-3.5 w-3.5" /> Prev
        </button>
        <span className="px-2">Page <span className="font-medium text-foreground">{page}</span> of {totalPages}</span>
        <button onClick={() => onPage(Math.min(totalPages, page + 1))} disabled={page >= totalPages} className="inline-flex items-center gap-1 rounded-lg border border-border bg-background px-2.5 py-1.5 hover:bg-accent disabled:opacity-50 disabled:cursor-not-allowed">
...
function ProductFormModal({ suppliers, product, onClose }: { suppliers: Supplier[]; product?: Product; onClose: () => void }) {
  const addProduct = useAddProduct();
  const updateProduct = useUpdateProduct();
  const isEdit = !!product;
  const [form, setForm] = useState<ProductForm>({
    name: product?.name ?? "", sku: product?.sku ?? "", category: product?.category ?? "Tops",
    supplier: product?.supplier ?? suppliers[0]?.name ?? "", material: product?.material ?? "",
    status: (product?.status as ComplianceStatus | undefined) ?? deriveStatusFromSupplier(suppliers[0]?.status),
  });
  const [errors, setErrors] = useState<Partial<Record<keyof ProductForm, string>>>({});

  const onSupplierChange = (name: string) => {
    const s = suppliers.find((x) => x.name === name);
    setForm((f) => ({ ...f, supplier: name, status: isEdit ? f.status : deriveStatusFromSupplier(s?.status) }));
  };
  ...
  const onSubmit = (e: React.FormEvent) => {
    ...
    setErrors({});
    if (isEdit) {
      updateProduct.mutate({ id: product!.id, ...result.data }, {
...
export default function ProductsPage() {
  ...
  const filtered = useMemo(() => {
    ...
    return products.filter((p) => {
      const matchQ = !term || p.name.toLowerCase().includes(term) || p.sku.toLowerCase().includes(term) || p.supplier.toLowerCase().includes(term);
      ...
      <div className="space-y-6 max-w-7xl">
        ...
        <div className="flex flex-wrap items-center gap-3 rounded-2xl border border-border bg-card p-3 shadow-sm">
          <div className="relative flex-1 min-w-[220px]">
            ...
            <input value={q} onChange={(e) => { setQ(e.target.value); setPage(1); }} placeholder="Search by name, SKU, supplier..." className="w-full rounded-xl border border-border bg-background pl-9 pr-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-ring" />
          </div>
          ...
          <select value={categoryFilter} onChange={(e) => { setCategoryFilter(e.target.value); setPage(1); }} className="rounded-xl border border-border bg-background px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-ring">
                ...
                {paged.map((p, idx) => {
                  ...
                  return (
                    <tr key={p.id} className="group transition-colors hover:bg-muted/40">
                      ...
                      <td className={`px-5 py-3.5 text-muted-foreground max-w-[220px] truncate ${idx !== paged.length - 1 ? "border-b border-border/60" : ""}`}>{p.material}</td>
                      <td className={`px-5 py-3.5 ${idx !== paged.length - 1 ? "border-b border-border/60" : ""}`}>
                        <div className="flex items-center gap-2">
                          <StatusBadge status={p.status} />
                          {issues.length > 0 && <span title={issues.join("\n")} className="text-amber-500"><AlertTriangle className="h-3.5 w-3.5" /></span>}
                        </div>
                      </td>
                      ...
                      <td className={`px-5 py-3.5 ${idx !== paged.length - 1 ? "border-b border-border/60" : ""}`}>
                        <div className="flex justify-end gap-1 opacity-60 group-hover:opacity-100 transition-opacity">
                          <button onClick={() => setEditing(p)} className="p-1.5 rounded-lg hover:bg-accent"><Pencil className="h-3.5 w-3.5" /></button>
                          <button onClick={() => { if (confirm(`Delete "${p.name}"?`)) deleteProduct.mutate(p.id, { onSuccess: () => toast.success("Product deleted"), onError: (err) => toast.error(err.message) }); }} className="p-1.5 rounded-lg hover:bg-destructive/10 text-destructive"><Trash2 className="h-3.5 w-3.5" /></button>
                        </div>
                      </td>
                    </tr>
                  );
                })}
                {paged.length === 0 && <tr><td colSpan={8} className="px-5 py-16 text-center text-muted-foreground">{isLoading ? "Loading..." : "No products found."}</td></tr>}
              </tbody>
            </table>
          </div>
