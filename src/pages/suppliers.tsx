...
import {
  ...
  type ComplianceStatus, type Supplier,
} from "@/lib/store";
...
const supplierSchema = z.object({
  ...
  certifications: z.array(z.string().trim().min(1).max(40)).max(20),
  status: statusEnum,
});
...
function SupplierFormModal({ supplier, onClose }: { supplier?: Supplier; onClose: () => void }) {
  ...
  const onSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    const payload = {
      name: form.name, country: form.country, contact: form.contact, email: form.email,
    ...
    setErrors({});
    if (isEdit) {
      updateSupplier.mutate({ id: supplier!.id, ...result.data }, {
...
export default function SuppliersPage() {
  ...
  const countries = useMemo(() => Array.from(new Set(suppliers.map((s) => s.country).filter(Boolean))).sort(), [suppliers]);

  const filtered = useMemo(() => {
    const term = q.toLowerCase();
    return suppliers.filter((s) => {
      const matchQ = !term || s.name.toLowerCase().includes(term) || s.contact.toLowerCase().includes(term) || s.email.toLowerCase().includes(term) || s.certifications.some((c) => c.toLowerCase().includes(term));
    ...
    <AppLayout>
      <div className="space-y-6 max-w-7xl">
        <div className="flex flex-wrap items-end justify-between gap-4">
          ...
          <button onClick={() => setCreating(true)} className="inline-flex items-center gap-2 rounded-xl bg-primary text-primary-foreground px-4 py-2 text-sm font-medium hover:bg-primary/90 shadow-sm transition">
        ...
        <div className="flex flex-wrap items-center gap-3 rounded-2xl border border-border bg-card p-3 shadow-sm">
          <div className="relative flex-1 min-w-[220px]">
            ...
            <input value={q} onChange={(e) => { setQ(e.target.value); setPage(1); }} placeholder="Search suppliers, contacts, certifications..." className="w-full rounded-xl border border-border bg-background pl-9 pr-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-ring" />
          </div>
          <div className="flex items-center gap-2 text-xs text-muted-foreground">
            <SlidersHorizontal className="h-3.5 w-3.5" /><span className="hidden sm:inline">Filters</span>
          </div>
          <select value={statusFilter} onChange={(e) => { setStatusFilter(e.target.value as "all" | ComplianceStatus); setPage(1); }} className="rounded-xl border border-border bg-background px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-ring">
          ...
          <select value={countryFilter} onChange={(e) => { setCountryFilter(e.target.value); setPage(1); }} className="rounded-xl border border-border bg-background px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-ring">
        ...
        <div className="rounded-2xl border border-border bg-card shadow-sm overflow-hidden">
          <div className="max-h-[640px] overflow-auto">
            <table className="w-full text-sm border-separate border-spacing-0">
              <thead className="sticky top-0 z-10 bg-muted/80 backdrop-blur text-muted-foreground">
                  ...
                  return (
                    <tr key={s.id} className="group transition-colors hover:bg-muted/40">
                      ...
                      <td className={`px-5 py-3.5 ${bd}`}>
                        <span className="inline-flex items-center gap-1 text-xs text-muted-foreground"><MapPin className="h-3 w-3" />{s.country || "—"}</span>
                      </td>
                      <td className={`px-5 py-3.5 ${bd}`}>
                        <div className="leading-tight">
                          <div>{s.contact || "—"}</div>
                          {s.email && <a href={`mailto:${s.email}`} className="text-xs inline-flex items-center gap-1 text-primary hover:underline"><Mail className="h-3 w-3" />{s.email}</a>}
                        </div>
                      </td>
                      <td className={`px-5 py-3.5 ${bd}`}>
                        <div className="flex flex-wrap gap-1 max-w-[280px]">
                          ...
                          {s.certifications.length > 4 && <span className="text-xs px-2 py-0.5 rounded-full bg-muted text-muted-foreground">+{s.certifications.length - 4}</span>}
                        </div>
                      </td>
                      <td className={`px-5 py-3.5 ${bd}`}><StatusBadge status={s.status} /></td>
                      <td className={`px-5 py-3.5 ${bd}`}>
                        <div className="flex justify-end gap-1 opacity-60 group-hover:opacity-100 transition-opacity">
                          <button onClick={() => setEditing(s)} className="p-1.5 rounded-lg hover:bg-accent"><Pencil className="h-3.5 w-3.5" /></button>
                          <button onClick={() => { if (confirm(`Delete supplier "${s.name}"?`)) deleteSupplier.mutate(s.id, { onSuccess: () => toast.success("Supplier deleted"), onError: (err) => toast.error(err.message) }); }} className="p-1.5 rounded-lg hover:bg-destructive/10 text-destructive"><Trash2 className="h-3.5 w-3.5" /></button>
                        </div>
                      </td>
                    </tr>
                  );
                })}
                {paged.length === 0 && <tr><td colSpan={6} className="px-5 py-16 text-center text-muted-foreground">{isLoading ? "Loading..." : "No suppliers found."}</td></tr>}
              </tbody>
            </table>
          </div>
          {filtered.length > 0 && (
            <div className="flex items-center justify-between gap-3 border-t border-border px-5 py-3 bg-muted/20 text-xs text-muted-foreground">
              <div>Showing <span className="font-medium text-foreground">{(currentPage - 1) * PAGE_SIZE + 1}–{Math.min(currentPage * PAGE_SIZE, filtered.length)}</span> of <span className="font-medium text-foreground">{filtered.length}</span></div>
              <div className="flex items-center gap-1">
                <button onClick={() => setPage(Math.max(1, currentPage - 1))} disabled={currentPage <= 1} className="inline-flex items-center gap-1 rounded-lg border border-border bg-background px-2.5 py-1.5 hover:bg-accent disabled:opacity-50 disabled:cursor-not-allowed">
                  <ChevronLeft className="h-3.5 w-3.5" /> Prev
                </button>
                <span className="px-2">Page <span className="font-medium text-foreground">{currentPage}</span> of {totalPages}</span>
                <button onClick={() => setPage(Math.min(totalPages, currentPage + 1))} disabled={currentPage >= totalPages} className="inline-flex items-center gap-1 rounded-lg border border-border bg-background px-2.5 py-1.5 hover:bg-accent disabled:opacity-50 disabled:cursor-not-allowed">
